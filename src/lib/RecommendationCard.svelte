			...
			{#if  result.data.Poster}
				<div in:fade class="relative flex flex-col md:flex-row bg-neutral-800/70 shadow-md p-6">
					<div
						...
						style={`background-image: url(${result.data.Poster})`}
					/>
					<div
					...
					<div class="z-40 flex flex-col justify-between md:ml-6 pt-32 md:pt-0">
						<div>
							<div class="flex items-end mb-4">
								<div class="font-bold text-slate-200 text-3xl">
									{result.data.Title}
									<span class="font-bold text-slate-200/60 text-xl ml-2">{result.data.Year}</span>
								</div>
							</div>
						...
						{#if result.data.channels.length>0}
							...
							<div class="flex items-center">播放源：
								{
									#each result.data.channels as channel, i(i)
								}
								<div class="mr-4 py-1 px-2 rounded-full  border border-pink-600 text-xs">
									<a  data-cn="{channel.channel}" href="{channel.url.play_link}" target="_blank">
									{channel.channel}
